...
export async function POST(request: Request) {
  ...
  if (body.packageId === 'projects') {
    ...
    lineItemDesc = `Backup von ${count} ChatGPT-Projekt${count === 1 ? '' : 'en'} - 1,20 EUR / Projekt`
    ...
    metaAppId = pkg.sessionOnly ? 'gpt-vault-session' : 'gpt-vault'
    maxGpts = pkg.gpts ?? 0
  }
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        invoice_nr: invoiceNr,
        max_gpts: String(maxGpts),
        max_projects: String(maxProjects),
      },
